  ...
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    ...
    <div class="container">
      <section>
        <div class="flex">
          <div class="max-w-xs">
            <label for="wallet" class="block text-sm font-medium text-gray-700">Тикер{{tiker}}</label>
            <div class="mt-1 relative rounded-md shadow-md">
              <input v-model="tiker" @keydown.enter="add" type="text" name="wallet" id="wallet"
      ...
      <template v-if="tikers.length">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div
           ...
           v-for="(t, index) in pagginateTikker" 
          :key="index" 
          @click="select(t)" 
          :class="selectedTiker === t ? 'border-4': ''"
        ...
        <section v-if="selectedTiker" class="relative">
          ...
          <div class="flex items-end border-gray-600 border-b border-l h-64">
            <div v-for="(bar, index) in normalizeGraph" :key="index" :style="{ height: `${bar}%` }"
              class="bg-purple-800 border w-10"></div>
          </div>
          <button @click="selectedTiker=null" type="button" class="absolute top-0 right-0">
...
export default {
  ...
  computed: {
    ...
    filterTikers() {
      return this.tikers.filter(t => t.name.includes(this.fillter));
    },
    
    pagginateTikker() {
      return this.filterTikers.slice(this.startPage, this.endPage);
    },

    hasNextPage() {
     return this.filterTikers.length > this.endPage;
    },
  ...
  methods: {
    ...
    handeTicker(removeTick) {
      this.tikers = this.tikers.filter(t => t != removeTick);
  ...
  watch: {
    ...
    pagginateTikker() {
      if (this.pagginateTikker.length === 0 && this.page > 1) {
        this.page -= 1;
      }
    },  
