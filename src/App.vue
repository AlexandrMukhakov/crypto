      ...
      <template v-if="tikers.length">
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div
           ...
           v-for="(t, index) in tikers" 
          :key="index" 
          @click="select(t)" 
          :class="sell === t ? 'border-4': ''"
...
export default {
  ...
  methods: {
    ...
    handeTicker(removeTick, index) {
      this.tikers = this.tikers.filter(t => t != removeTick);
      const localTiker = localStorage.getItem('cryptonomicon');
      localStorage.removeItem(localTiker[index])
    },
